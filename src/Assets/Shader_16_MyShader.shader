Shader "Custom/Shader_16_MyShader"
{
    Properties
    {
        ...
        _RoughnessX("RoughnessX", Range(0, 1)) = 0.5
        _RoughnessY("RoughnessY", Range(0, 1)) = 0.2
        _RoughnessX2("RoughnessX2", Range(0, 1)) = 0.2
        _RoughnessY2("RoughnessY2", Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half alphaX2 = _RoughnessX2 * _RoughnessX2;
                half alphaY2 = _RoughnessY2 * _RoughnessY2;
                ...
                half c2 = (XdotH * XdotH / (alphaX2 * alphaX2) + YdotH * YdotH / (alphaY2 * alphaY2)) / (HdotN * HdotN);
                half3 specular = _SpecularIntensity * exp(-c) / sqrt(LdotN * VdotN) / (4 * PI * alphaX * alphaY);
                half3 specular2 = _SpecularIntensity * exp(-c2) / sqrt(LdotN * VdotN) / (4 * PI * alphaX2 * alphaY2);

                half3 color = light.color * lerp(lerp(lambert, ambient, _AmbientRate), specular, _Metallic);
                half3 color2 = light.color * lerp(lerp(lambert, ambient, _AmbientRate), specular2, _Metallic);
                return half4(color + color2, 1);
            }
            ENDHLSL
        }
    }
